...
import com.lei.Output2File;
import com.lei.Poisson;
...
public class Datacenter extends SimEntity {
	...
	public void processEvent(SimEvent ev) {
		...
		// Resource characteristics inquiry
			...
			case CloudSimTags.VM_DATACENTER_EVENT:
				...
				if (!this.getVmList().isEmpty() && Config.isTimeoutHappen){
					List<Vm> vms = this.getVmList();
					for (Vm vm : vms) {
						CloudletSchedulerDynamicWorkload cs = (CloudletSchedulerDynamicWorkload) vm.getCloudletScheduler();
						List<ResCloudlet> rcls = cs.getCloudletExecList();
						for(ResCloudlet rcl : rcls) {
							Cloudlet cloudlet = rcl.getCloudlet();
							if(rcl.getCloudletFinishedSoFar() > cloudlet.getCloudletLength()/2) {
								double timeoutClock = rcl.getCloudletFinishedSoFar()/(Consts.MILLION*cs.getTotalMips()*Config.timeoutCpuUtilization);
								cloudlet.TimeoutSimulation(timeoutClock);
							}
	...
	protected void processVmDestroy(SimEvent ev, boolean ack) {
		...
		if (ack) {
			...
			data[1] = vm.getId();
			data[2] = CloudSimTags.TRUE;
			//@lei 2018.1.8 改sendNow为send，增加delay=500
	...
	//@lei 2017.12.25
	public boolean detectHostStatus(Host host) {
		DecimalFormat df = new DecimalFormat("#.#######");
		boolean status = host.status;
		double randnum = Config.randnum;
		double x1 = 0;
		double x2 = 1;
		double f = Math.random()/Math.nextDown(1.0);
		double rand = x1*(1.0 - f) + x2*f;
		double fr = 1 - Poisson.PMF(Config.lambda, CloudSim.clock()/10000, 0);
		if(rand<0.95) {
			fr *= 0.01;
		}
		if(status) {
		...
		String file = Config.path+Config.filename;
		String data = host.getId() + Config.comma + 
				df.format(fr) + Config.comma +
				status + Config.comma+ CloudSim.clock() + Config.LineSep;
				
		if(Output2File.createFile(file)) {
			Output2File.writeFile(file, data);
		}
