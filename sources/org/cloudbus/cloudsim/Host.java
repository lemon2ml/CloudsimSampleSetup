...
public class Host {
	...
	//	@lei 2018.1.3
	public void detectVmStatus(List<? extends Vm> vmlist) {
		DecimalFormat df = new DecimalFormat("#.#######");
		for(Vm vm : vmlist) {
			boolean status = vm.status;
			double x1 = 0;
			double x2 = 1;
			double f = Math.random()/Math.nextDown(1.0);
			double rand = x1*(1.0 - f) + x2*f;
			double fr = 1 - Poisson.PMF(Config.lambda_vm,CloudSim.clock()/1000,0);
			if(rand<0.95) {
				fr*=0.01;
			}
			if(status) {
				if(rand < fr) {
					vm.status  = false;
					Config.vmCount++;
//					System.out.println("Config.vmCount="+Config.vmCount+" hostid="+id+" vmid=" + vm.getId() + " fr=" + fr+" rand="+rand);
				}
			}
			String file = Config.path+Config.filename_vm;
			String data = id+Config.comma+vm.getId() + Config.comma + 
