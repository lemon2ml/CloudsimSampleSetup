...
public class CloudSim {
	...
	public static boolean runClockTick() {
		...
		for (int i = 0; i < entities_size; i++) {
			...
			//@lei 2017.12.25
			if((ent.getName()=="Datacenter" || ent.getName()=="Datacenter_1") && CloudSim.clock()>ct) {
				Datacenter dc = (Datacenter)ent;
				hosts = dc.getHostList();
				...
				for(int j=0;j<hostSize;j++) {
					...
					host.detectVmStatus(vms);
					
					if(host.status) {
						host.status = dc.detectHostStatus(host);
					}
					...
					for(Vm vm : vms) {
						...
						for(int k=0;k<historySize;k++) {
							double allocatedMips = vm.getStateHistory().get(k).getAllocatedMips();
				...
				ct = CloudSim.clock();
				if(countTotalMips >= Config.SIMULATION_LIMIT_MIPS && Config.isLimitMips) {
					terminateSimulation(ct+1);
					System.out.println("\ncountTotalMips="+countTotalMips );
				}
			}
		}
	...
	public static void send(int src, int dest, double delay, int tag, Object data) {
		...
		e.CustomtoString("future queue added\n");
		future.addEvent(e);
	}
