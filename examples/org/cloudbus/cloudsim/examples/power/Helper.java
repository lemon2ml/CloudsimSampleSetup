...
public class Helper {
	...
	public static void printResults(
		...
		double countTotalMips = 0;
		for(Vm vm : vms) {
			double countVmMips = 0;
			int historySize =  vm.getStateHistory().size();
			for(int i=0;i<historySize;i++) {
				double allocatedMips = vm.getStateHistory().get(i).getAllocatedMips();
				countVmMips += allocatedMips;
			}
			countTotalMips += countVmMips;
		}
...
//			data.append(String.format("%.10f", slaAverage) + delimeter);
			...
			double slaViolation = slaAverage;
			data.append(String.format("%.10f", slaViolation) + delimeter);
...
//				data.append(String.format("%.5f", executionTimeTotalStDev) + delimeter);
			...
			double hostErrCount = Config.hostCount;
			double vmErrCount = Config.vmCount;
			data.append(String.format("%.10f", hostErrCount) + delimeter);
			data.append(String.format("%.10f", vmErrCount) + delimeter);
			if(Config.isLimitMips) {
				data.append(String.format("%.10f", Config.SIMULATION_LIMIT_MIPS) + delimeter);
			}else {
				data.append(String.format("%.10f", countTotalMips) + delimeter);
			}
			
			data.append("\n");

			System.out.println("Config.hostCount="+Config.hostCount);
			System.out.println("Config.vmCount=" + Config.vmCount);
			
			writeDataRow(data.toString(), outputFolder + "/stats/" + experimentName + "_stats_"+new java.util.Date().getTime()+".csv");
	...
	public static void printResults(
		...
		int numberOfHosts = hosts.size();
		int numberOfVms = vms.size();
		int numberOfCloudlets = cloudlets.size();
		...
		for(Cloudlet cloudlet : cloudlets) {
			if(cloudlet.isDataIC) {
				countDataIC++;
			}
			
			if(cloudlet.isTimeout && Math.random()<0.2) {
				countTimeout++;
			}
		}
		System.out.println("countDataIC=" + countDataIC);
		System.out.println("countTimeout=" + countTimeout);
		...
		if (outputInCsv) {
			...
			data.append(String.format("%d", numberOfHosts) + delimeter);
			data.append(String.format("%d", numberOfVms) + delimeter);
			data.append(String.format("%d", numberOfCloudlets) + delimeter);
			data.append(String.format("%.2f", totalSimulationTime) + delimeter);
			data.append(String.format("%.5f", energy) + delimeter);
			data.append(String.format("%d", numberOfMigrations) + delimeter);
			data.append(String.format("%.10f", sla) + delimeter);
...
//				data.append(String.format("%.5f", executionTimeTotalStDev) + delimeter);
			...
			data.append(String.format("%.10f", hostErrCount) + delimeter);
			data.append(String.format("%.10f", vmErrCount) + delimeter);
			data.append(String.format("%d", countDataIC) + delimeter);
			data.append(String.format("%d", countTimeout) + delimeter);
			if(Config.isLimitMips) {
				data.append(String.format("%.10f", Config.SIMULATION_LIMIT_MIPS) + delimeter);
			}else {
				data.append(String.format("%.10f", countTotalMips) + delimeter);
